...
function Permissions({ onBack, onHome }) {
  ...
  const coffeeSiteTool         = tools.find(t => t.slug === 'coffee-site');
  const spiritsOrderingTool    = tools.find(t => t.slug === 'spirits-ordering');
  const dualSlugs     = ['hr-documents', 'sops', 'checklists', 'taproom-inventory', 'recipes', 'cocktail-keeper', 'sales-crm', 'label-inventory', 'production-schedule', '86ed-customers', 'distro-taproom-orders', 'coffee-keeper', 'production-weekly', 'equipment-manuals', 'tank-maintenance', 'distillery-inventory', 'coffee-site', 'spirits-ordering'];
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
                ...
                <>
                  <tr className="border-b border-gray-700/50">
                    ...
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-3 pb-1 text-center">
                        <Toggle
                          active={hasPermission(role, eightySixedTool.id, 'view')}
                          onClick={() => togglePermission(role, eightySixedTool.id, 'view')}
                        />
                      </td>
                    ))}
                  </tr>
              ...
              {/* Coffee Keeper — two rows: Access and Manage */}
              {coffeeTool && (
                <>
                  <tr className="border-b border-gray-700/50">
                    <td className="px-3 pt-3 pb-1 whitespace-nowrap">
                      <div className="text-white text-sm font-medium">Coffee Keeper</div>
                      <div className="text-gray-500 text-xs mt-0.5">Access</div>
                    </td>
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-3 pb-1 text-center">
                        <Toggle
                          active={hasPermission(role, coffeeTool.id, 'view')}
                          onClick={() => togglePermission(role, coffeeTool.id, 'view')}
                        />
                      </td>
                    ))}
                  </tr>
              ...
              {/* Equipment Manuals — two rows: Access and Manage */}
              {equipmentTool && (
                <>
                  <tr className="border-b border-gray-700/50">
                    <td className="px-3 pt-3 pb-1 whitespace-nowrap">
                      <div className="text-white text-sm font-medium">Equipment Manuals</div>
                      <div className="text-gray-500 text-xs mt-0.5">Access</div>
                    </td>
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-3 pb-1 text-center">
                        <Toggle
                          active={hasPermission(role, equipmentTool.id, 'view')}
                          onClick={() => togglePermission(role, equipmentTool.id, 'view')}
                        />
                      </td>
                    ))}
                  </tr>
              ...
              {/* Spirits Ordering — two rows: Access and Manage */}
              {spiritsOrderingTool && (
                <>
                  <tr className="border-b border-gray-700/50">
                    <td className="px-3 pt-3 pb-1 whitespace-nowrap">
                      <div className="text-white text-sm font-medium">Spirits Ordering</div>
                      <div className="text-gray-500 text-xs mt-0.5">Access</div>
                    </td>
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-3 pb-1 text-center">
                        <Toggle
                          active={hasPermission(role, spiritsOrderingTool.id, 'view')}
                          onClick={() => togglePermission(role, spiritsOrderingTool.id, 'view')}
                        />
                      </td>
                    ))}
                  </tr>
                  <tr className="border-b border-gray-700">
                    ...
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-1 pb-3 text-center">
                        <Toggle
                          active={hasPermission(role, spiritsOrderingTool.id, 'upload')}
                          onClick={() => togglePermission(role, spiritsOrderingTool.id, 'upload')}
                        />
                      </td>
                    ))}
                  </tr>
              ...
              {/* Coffee Site — two rows: Access and Manage */}
              {coffeeSiteTool && (
                <>
                  <tr className="border-b border-gray-700/50">
                    <td className="px-3 pt-3 pb-1 whitespace-nowrap">
                      <div className="text-white text-sm font-medium">Coffee Site</div>
                      <div className="text-gray-500 text-xs mt-0.5">Access</div>
                    </td>
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-3 pb-1 text-center">
                        <Toggle
                          active={hasPermission(role, coffeeSiteTool.id, 'view')}
                          onClick={() => togglePermission(role, coffeeSiteTool.id, 'view')}
                        />
                      </td>
                    ))}
                  </tr>
                  <tr className="border-b border-gray-700">
                    ...
                    {ROLES.map((role) => (
                      <td key={role} className="px-1 pt-1 pb-3 text-center">
                        <Toggle
                          active={hasPermission(role, coffeeSiteTool.id, 'upload')}
                          onClick={() => togglePermission(role, coffeeSiteTool.id, 'upload')}
                        />
                      </td>
                    ))}
                  </tr>
