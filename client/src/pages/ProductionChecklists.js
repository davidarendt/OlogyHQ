...
function RunModal({ checklist, onClose }) {
  ...
  useEffect(() => {
    fetch(`${API}/api/production-checklists/${checklist.id}/today`, { credentials: 'include' })
      .then(r => r.json())
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`prod-checklist-daily-${checklist.id}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'production_checklist_daily_state',
  ...
  const toggle = (id) => {
    ...
    fetch(`${API}/api/production-checklists/${checklist.id}/items/${id}/check`, { method, credentials: 'include' })
...
function ChecklistModal({ checklist, onClose, onSaved }) {
  ...
  const [category, setCategory]   = useState(checklist?.category || 'daily');
  ...
  const handleSave = async () => {
    ...
    const body = { name: name.trim(), display_name: displayName.trim() || null, category, description: description.trim(), frequency, roles, items: validItems };
    const url = isEdit ? `${API}/api/production-checklists/${checklist.id}` : `${API}/api/production-checklists`;
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Daily Brew Check"
            ...
            <div>
              ...
              <input value={displayName} onChange={e => setDispName(e.target.value)} placeholder="e.g. Daily Check"
                className="w-full bg-gray-700 text-white px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
          </div>
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function ProductionChecklists({ user, canUpload, onBack }) {
  ...
  const fetchAll = async () => {
    const [clRes, runRes] = await Promise.all([
      fetch(`${API}/api/production-checklists`, { credentials: 'include' }),
      fetch(`${API}/api/production-checklists/runs`, { credentials: 'include' }),
    ]);
  ...
  const handleDeleteChecklist = async id => {
    ...
    await fetch(`${API}/api/production-checklists/${id}`, { method: 'DELETE', credentials: 'include' });
    setChecklists(p => p.filter(c => c.id !== id));
    setRuns(p => p.filter(r => r.checklist_id !== id));
  };

  const handleDeleteRun = async id => {
    await fetch(`${API}/api/production-checklists/runs/${id}`, { method: 'DELETE', credentials: 'include' });
  ...
  const moveChecklist = async (index, direction) => {
    ...
    setChecklists(next);
    await fetch(`${API}/api/production-checklists/reorder`, {
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h2 className="text-cream text-4xl font-bold">Production Checklists</h2>
            <p className="text-gray-400 mt-2">Brewery production checklists</p>
          </div>
        ...
        {loading ? (
          ...
          checklists.length === 0 ? (
            <div className="text-center py-20 text-gray-600 text-sm">No checklists available.</div>
          ) : (
